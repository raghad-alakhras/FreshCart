...
import { CiDeliveryTruck, CiLogout, CiMail, CiSearch, CiUser } from "react-icons/ci";
      ...
      <div className="px-5 flex gap-6 items-center justify-between border-t border-gray-300 pt-3">
        ...
        <div className="logo">
          ...
          >
            ...
            <defs>
              <clipPath id="clip0_16_7258">
                <rect width="165.16" height="32" fill="white" />
              </clipPath>
              <clipPath id="clip1_16_7258">
                <rect width="165.16" height="32" fill="white" />
              </clipPath>
            </defs>
          </svg>
        ...
        {/* search bar */}
        <form onSubmit={handleSubmit(handleSearch)} className="relative hidden lg:block w-fit">
          <input
          ...
          <NavigationMenuList className="hidden xl:flex">
            {links.map((link) => (
              <NavigationMenuItem key={link.path} >
                ...
                >
                  <Link className="text-[16px]! hover:text-green-600! hover:bg-transparent! focus:bg-transparent! text-gray-700!" href={link.path}>{link.element}</Link>
                </NavigationMenuLink>
              </NavigationMenuItem>
            ))}

            <NavigationMenuItem  className="hidden xl:flex xl:w-full">
              <NavigationMenuTrigger className="text-[16px]! hover:text-green-600! hover:bg-transparent! focus:bg-transparent! text-gray-700!">Categories</NavigationMenuTrigger>
              <NavigationMenuContent  >
                <ul >
        ...
        {status==='authenticated'&&<>
         <div className="flex items-center gap-2">
          <Link href={'/wishlist'} className="flex items-center relative justify-center size-8 rounded-full group hover:bg-green-50 transition-all duration-300">
            <IoIosHeartEmpty className="text-xl text-gray-600 group-hover:text-green-600 transition-all duration-300" />
           {wishlistData?.count ? <span className="size-4 text-xs rounded-full flex items-center justify-center text-white bg-red-600 border border-white absolute top-0 right-0">{wishlistData?.count}</span>:null}
          </Link>
        
          <Link href={'/cart'} className="flex items-center justify-center size-8 rounded-full group hover:bg-green-50 transition-all duration-300 relative">
            <FaShoppingCart className="text-xl text-gray-600 group-hover:text-green-600 transition-all duration-300" />
          {cartData?.numOfCartItems ? <span className="size-4 text-xs rounded-full flex items-center justify-center text-white bg-green-600 border border-white absolute top-0 right-0">{cartData?.numOfCartItems}</span>:null}
          </Link>
          <Link href={'/profile/accountAddress'} className="flex items-center justify-center size-8 rounded-full group hover:bg-green-50 transition-all duration-300 relative">
            <CgProfile  className="text-xl text-gray-600 group-hover:text-green-600 transition-all duration-300" />
          </Link>
          <button onClick={handleLogout} className="hidden lg:flex px-4 py-2 bg-green-600 text-white  rounded-full items-center gap-1 hover:bg-green-700 transition-all duration-300">
           <CiLogout /> Logout
          </button>
        </div></>}
