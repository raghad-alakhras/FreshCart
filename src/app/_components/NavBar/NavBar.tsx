...
import { FaHeadphones, FaShoppingCart, FaUserPlus } from "react-icons/fa";
...
export default function NavBar() {
  ...
  const { register, handleSubmit } = useForm<{ productName: string }>();

  function handleSearch(data: { productName: string }) {
    setOpenMenu(false)
  ...
  return (
    <nav className="relative">
      <div className="flex items-center justify-between my-3 hidden lg:flex pb-1">
        ...
        <div className="flex items-center gap-5">
          <div className="flex items-center gap-5 pr-4 border-r border-gray-200">
            <a
              href="tel:+1123-456"
              className="text-gray-500 text-sm flex items-center gap-2 hover:text-green-600 transition-colors duration-200"
            >
              <BsFillTelephoneFill />
              +1(800) 123-4567
            </a>
          ...
          {status !== "authenticated" && (
            <>
              <div className="flex items-center gap-4 ">
                <Link
                  href={`/login`}
                  className=" flex items-center gap-3 text-gray-500 hover:text-green-600 text-sm transition-colors duration-200"
                >
                  <CiUser />
                  <span>Signin</span>
                </Link>
      ...
      <div className="px-5 flex gap-6 items-center justify-between border-t border-gray-300 pt-3">
        {/* logo */}
        <div className="logo">
          <svg
            width="166"
        ...
        <form
          ...
          className="relative hidden lg:block w-fit"
        >
          <input
            {...register("productName")}
            type="text"
            placeholder="Search for products,brands and more..."
            className="w-100 p-3 rounded-full border border-gray-300 focus:border-green-600 focus:ring-2 focus:ring-green-500"
          />
          <div className="absolute right-1 top-1/2 -translate-y-1/2 size-9 rounded-full text-white bg-green-600 flex items-center justify-center">
          ...
          <NavigationMenuList className="hidden xl:flex">
            {links.map((link) => (
              <NavigationMenuItem key={link.path}>
                ...
                >
                  <Link
                    className="text-[16px]! hover:text-green-600! hover:bg-transparent! focus:bg-transparent! text-gray-700!"
                    href={link.path}
                  >
                    {link.element}
                  </Link>
            ...
            <NavigationMenuItem className="hidden xl:flex xl:w-full">
              <NavigationMenuTrigger className="text-[16px]! hover:text-green-600! hover:bg-transparent! focus:bg-transparent! text-gray-700!">
                Categories
              </NavigationMenuTrigger>
              <NavigationMenuContent>
                <ul>
        ...
        {status === "authenticated" && (
          <>
            <div className="flex items-center gap-2">
              <Link
                href={"/wishlist"}
                className="sm:flex hidden items-center relative justify-center size-8 rounded-full group hover:bg-green-50 transition-all duration-300"
              >
                <IoIosHeartEmpty className="text-xl text-gray-600 group-hover:text-green-600 transition-all duration-300" />
                {wishlistData?.count ? (
                  <span className="size-4 text-xs rounded-full flex items-center justify-center text-white bg-red-600 border border-white absolute top-0 right-0">
                    {wishlistData?.count}
                  </span>
                ) : null}
              </Link>
              ...
              <Link
                href={"/cart"}
                className="sm:flex hidden items-center justify-center size-8 rounded-full group hover:bg-green-50 transition-all duration-300 relative"
              >
                <FaShoppingCart className="text-xl text-gray-600 group-hover:text-green-600 transition-all duration-300" />
                {cartData?.numOfCartItems ? (
                  <span className="size-4 text-xs rounded-full flex items-center justify-center text-white bg-green-600 border border-white absolute top-0 right-0">
                    {cartData?.numOfCartItems}
                  </span>
                ) : null}
              </Link>
              ...
              className="flex sm:hidden items-center justify-center size-8 rounded-full group hover:bg-green-600 transition-all duration-300 relative">
                <HiMiniBars3 className="text-lg text-gray-600 group-hover:text-white transition-all duration-300" />
              </div>
              <Link
                href={"/profile/accountAddress"}
                className="md:flex hidden items-center justify-center size-8 rounded-full group hover:bg-green-50 transition-all duration-300 relative"
              >
                <CgProfile className="text-xl text-gray-600 group-hover:text-green-600 transition-all duration-300" />
              </Link>
              <button
                onClick={handleLogout}
                className="hidden lg:flex px-4 py-2 bg-green-600 text-white  rounded-full items-center gap-1 hover:bg-green-700 transition-all duration-300"
              >
                <CiLogout /> Logout
              </button>
            </div>
      ...
      {openMenu && (
        ...
        <div className="absolute top-0 bottom-0 left-0 right-0 z-100 bg-white min-h-screen p-6">
         {/* icons */}
          <div className="flex items-center justify-between ">
           ...
           className="size-9 rounded-full bg-gray-100 text-gray-900 flex items-center justify-center text-lg">
              <MdCancel />
           </div>
        ...
        >
          <input
            ...
            className="w-[100%] block p-3 rounded-xl border border-gray-300 focus:border-green-600 focus:ring-2 focus:ring-green-500"
          />
          <div className="absolute right-1 top-1/2 -translate-y-1/2 size-9 rounded-xl text-white bg-green-600 flex items-center justify-center">
            <CiSearch className="text-lg" />
          </div>
        </form>
          </div>
          {/* list links */}
             <NavigationMenu className="block pt-6 border-t border-gray-100">
          <NavigationMenuList className="flex-col  items-start gap-4">
            {links.map((link) => (
              <NavigationMenuItem key={link.path}>
                ...
                >
                  <Link
                    className="text-[16px]! text-left hover:text-green-600! hover:bg-transparent! focus:bg-transparent! text-gray-700!"
                    href={link.path}
                  >
                    {link.element}
                  </Link>
          ...
          {/* wishlist icons */}
          <div className="flex items-center justify-between my-5 pt-6 border-t border-gray-100">
            <Link href={"/wishlist"} className="flex items-center gap-5">
               <div
                
                className="flex items-center justify-center size-9 rounded-full bg-red-50 transition-all duration-300"
              >
                <IoIosHeartEmpty className="text-xl text-red-600 transition-all duration-300" />
                
              </div>
              <span className="text-gray-700 hover:text-red-600">Wishlist</span>
            </Link>
                 
            {wishlistData?.count ? (
                  <span className="w-7 h-6 text-xs rounded-lg flex items-center justify-center text-white bg-red-600 border border-white">
                    {wishlistData?.count}
                  </span>
                ) : null}
          </div>
          ...
          <div className="flex items-center justify-between my-5">
            <Link  href={"/cart"} className="flex items-center gap-5">
               <div
                ...
                className="flex  items-center justify-center size-9 rounded-full bg-green-50 transition-all duration-300 "
              >
                <FaShoppingCart className="text-xl text-green-600 transition-all duration-300" />
               
              </div>
              <span className="text-gray-700 hover:text-green-600">Cart</span>
            </Link>
                 
            {cartData?.numOfCartItems ? (
                  <span className="w-7 h-6 text-xs rounded-lg flex items-center justify-center text-white bg-green-600 border border-white">
                    {cartData?.numOfCartItems}
                  </span>
                ) : null}
          </div>
          {/* user */}
          <Link  href={"/profile/accountAddress"} className="flex items-center gap-5 pt-6 border-t border-gray-100">
               <div
               
                className="flex  items-center justify-center size-9 rounded-full bg-gray-200 transition-all duration-300 "
              >
                <CiUser  className="text-xl text-gray-600 transition-all duration-300" />
               
              </div>
              <span className="text-gray-700 hover:text-green-600">{data?.user?.name}</span>
            </Link>
          {/* logout */}
          <div  onClick={handleLogout} className="flex items-center gap-5 mt-6">
               <div
               
                className="flex  items-center justify-center size-9 rounded-full bg-gray-200 transition-all duration-300 "
              >
                <BiLogOut   className="text-xl text-red-600 transition-all duration-300" />
